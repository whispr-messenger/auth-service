...
- **Support multidevices**: Fonctionnement cohérent à travers plusieurs appareils d'un même utilisateur
...
7. **Système de prékeys**: Permettant l'établissement asynchrone de sessions
...
Le protocole Signal utilise plusieurs types de clés cryptographiques:
...
1. **Clé d'identité (Identity Key)**: 
   ...
   - La clé publique est partagée avec les autres utilisateurs
...
2. **Clés préalables signées (Signed Prekeys)**:
   ...
   - Stockées sur le serveur avec la signature
...
3. **Clés préalables à usage unique (One-Time Prekeys)**:
   ...
   - Supprimées du serveur après utilisation
...
sequenceDiagram
    ...
    Note over B,S: Phase préliminaire (Bob est en ligne)
    ...
    B->>S: Envoie IKb(public), SPKb(public), Signature, OPKs(public)
    S->>S: Stocke les clés publiques de Bob
    
    Note over A,S: Phase d'initiation (Bob peut être hors ligne)
    A->>A: Génère IKa (Clé d'identité)
    A->>A: Génère EKa (Clé éphémère)
    A->>S: Demande les clés publiques de Bob
    S->>A: Envoie IKb(public), SPKb(public), Signature, OPK(public)
    ...
    A->>S: Envoie message initial chiffré avec SK, IKa(public), EKa(public)
    S->>S: Marque OPK comme utilisée (si utilisée)
    
    Note over B,A: Phase de réception (quand Bob se connecte)
    S->>B: Transmet message initial, IKa(public), EKa(public)
    B->>B: Retrouve OPK utilisée (si applicable)
...
1. **Phase préliminaire** (préparation du destinataire):
   - Le destinataire génère et enregistre ses clés sur le serveur
...
2. **Phase d'initiation** (par l'expéditeur):
   - L'expéditeur récupère les clés publiques du destinataire
...
3. **Phase de réception** (par le destinataire):
   - Le destinataire récupère le message et les clés de l'expéditeur
...
stateDiagram-v2
    ...
    Génération --> Utilisation: Première connexion
...
1. **Génération**:
   ...
   - Publiée sur le serveur (clé publique uniquement)
...
3. **Vérification**:
   - Les utilisateurs peuvent vérifier manuellement les clés d'identité
   - Cette vérification se fait via des codes de sécurité ou QR codes
   - Marque les clés comme "vérifiées" pour plus de sécurité
...
5. **Migration**:
   ...
   - Avertissement aux contacts sur le changement de clé
...
sequenceDiagram
    ...
    D->>S: Téléverse SPK(public) + signature
    S->>S: Stocke la nouvelle SPK avec horodatage
    
    Note over D,S: Après une période définie (7 jours par défaut)
    
    D->>D: Génère une nouvelle paire SPK'
    D->>D: Signe SPK' avec la clé d'identité
    D->>S: Téléverse SPK'(public) + signature
    S->>S: Conserve SPK pendant période de transition
    S->>S: Marque SPK' comme active
    
    Note over D,S: Après période de transition (1 jour par défaut)
    
    S->>S: Supprime l'ancienne SPK
...
1. **Cycle de rotation**:
   ...
   - Stockées sur le serveur avec leur signature

2. **Période de transition**:
   - Le serveur conserve l'ancienne et la nouvelle clé pendant une période définie
...
sequenceDiagram
    ...
    D->>S: Téléverse 100 OPKs (clés publiques uniquement)
    S->>S: Stocke les OPKs avec identifiants uniques
    
    Note over D,S: Lorsqu'un expéditeur établit une nouvelle session
    
    S->>S: Sélectionne et marque une OPK comme utilisée
    S->>S: Fournit l'OPK à l'expéditeur
    
    Note over D,S: Lorsque le nombre d'OPKs disponibles est faible
    
    S->>D: Notification "OPKs faibles" (< 20 disponibles)
    D->>D: Génère 100 nouvelles OPKs
    D->>S: Téléverse les nouvelles OPKs
    S->>S: Ajoute les nouvelles OPKs au pool disponible
```

1. **Génération par lot**:
   - Générées en grands nombres (typiquement 100 à la fois)
   - Seules les clés publiques sont téléversées vers le serveur
   - Les clés privées restent exclusivement sur l'appareil

2. **Utilisation unique**:
   - Chaque clé est fournie à un seul expéditeur
   - Marquée comme utilisée sur le serveur
   - Ne peut jamais être réutilisée pour une autre session

3. **Renouvellement**:
   - Le serveur surveille le nombre de clés disponibles
...
1. **En-tête** (non chiffré):
   ...
   - Identifiants du destinataire et de l'expéditeur
...
graph TD
    ...
    H --> I[Construire message formaté]
...
Le protocole Signal traite chaque paire appareil-appareil comme une session cryptographique distincte:
...
1. **Sessions indépendantes**:
   ...
   - Une session est identifiée par une paire (appareil émetteur, appareil destinataire)
...
sequenceDiagram
    ...
    A1->>S: Envoie message chiffré pour B1
    
    A1->>A1: Chiffre une copie du message pour A2
    A1->>S: Envoie copie synchronisée pour A2
    
    S->>B1: Délivre message à B1
    S->>A2: Délivre copie synchronisée à A2
...
1. **Partage des messages**:
   ...
   - Cette copie est envoyée via le serveur comme "message de synchronisation"
...
2. **Chiffrement des copies**:
   ...
   - Garantit que même le serveur ne peut pas lire les messages synchronisés
...
sequenceDiagram
    ...
    A1->>A1: Génère code de sécurité basé sur sa clé d'identité
    A2->>A2: Génère code de sécurité basé sur sa clé d'identité
    ...
    alt Codes identiques
        ...
        A1->>A1: Marque l'appareil 2 comme vérifié
        A2->>A2: Marque l'appareil 1 comme vérifié
...
3. **Contre-mesures supplémentaires**:
...
- Établissement complet de sessions X3DH
- Échange de messages bidirectionnel
- Rotation des clés préalables
- Synchronisation multi-appareils
...
- **Service de gestion des clés**: Génération et rotation des clés
...
- **Module de synchronisation**: Gestion de la cohérence multi-appareils
...
| PreKey | Clé préalable permettant l'établissement asynchrone de session |
...
| Attaque par homme du milieu | Interception active | Vérification des clés d'identité |
