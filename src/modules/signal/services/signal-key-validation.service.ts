...
export class SignalKeyValidationService {
	...
	validatePublicKeyFormat(publicKey: string): void {
		...
		if (publicKey.length < this.MIN_KEY_LENGTH) {
			throw new BadRequestException(`Public key too short (minimum ${this.MIN_KEY_LENGTH} characters)`);
		}

		if (publicKey.length > this.MAX_KEY_LENGTH) {
			throw new BadRequestException(`Public key too long (maximum ${this.MAX_KEY_LENGTH} characters)`);
		}
	}
	...
	validatePreKeys(preKeys: PreKeyDto[]): void {
		...
		if (preKeys.length > 200) {
			throw new BadRequestException('Cannot upload more than 200 prekeys at once');
		}
		...
		for (const preKey of preKeys) {
			...
			if (keyIds.has(preKey.keyId)) {
				throw new BadRequestException(`Duplicate keyId found in batch: ${preKey.keyId}`);
			}
	 ...
	 */
	async validateSignedPreKeyIdUniqueness(userId: string, deviceId: string, keyId: number): Promise<void> {
		const isUnique = await this.isSignedPreKeyIdUnique(userId, deviceId, keyId);

		if (!isUnique) {
			throw new BadRequestException(`Signed prekey with keyId ${keyId} already exists for this device`);
		}
	}
	 ...
	 */
	validateKeyBundle(identityKey: string, signedPreKey: SignedPreKeyDto, preKeys: PreKeyDto[]): void {
