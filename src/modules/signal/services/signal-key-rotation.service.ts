...
export class SignalKeyRotationService {
	 ...
	 */
	async replenishPreKeys(userId: string, deviceId: string, newPreKeys: PreKeyDto[]): Promise<void> {
		...
		try {
			...
			this.logger.error(`Failed to replenish prekeys for user ${userId}`, error.stack);
			throw error;
		}
	}
	...
	async getRotationRecommendations(
		...
		const [needsPreKeys, needsRotation, availablePreKeys, signedPreKey] = await Promise.all([
