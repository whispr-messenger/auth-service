...
export class SignalKeyStorageService {
	...
	async storeSignedPreKey(
		...
		try {
			...
			this.logger.error(`Failed to store signed prekey for user ${userId}`, error.stack);
			throw error;
		}
	}
	...
	async getIdentityKey(userId: string, deviceId: string): Promise<IdentityKey | null> {
		...
		try {
			...
			this.logger.error(`Failed to retrieve identity key for user ${userId}`, error.stack);
			throw error;
		}
	}
	...
	async getActiveSignedPreKey(userId: string, deviceId: string): Promise<SignedPreKey | null> {
		...
		try {
			...
			this.logger.error(`Failed to retrieve active signed prekey for user ${userId}`, error.stack);
			throw error;
		}
	}
	...
	async getUnusedPreKey(userId: string, deviceId: string): Promise<PreKey | null> {
		...
		try {
			...
			this.logger.error(`Failed to retrieve unused prekey for user ${userId}`, error.stack);
			throw error;
		}
	}
	...
	async getUnusedPreKeyCount(userId: string, deviceId: string): Promise<number> {
		...
		try {
			...
			this.logger.error(`Failed to count unused prekeys for user ${userId}`, error.stack);
			throw error;
		}
	}
