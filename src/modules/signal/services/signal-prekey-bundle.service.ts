...
export class SignalPreKeyBundleService {
	 ...
	 */
	async getBundleForUser(userId: string, deviceId: string): Promise<KeyBundleResponseDto> {
	...
	async getPreKeyStatus(userId: string, deviceId: string): Promise<PreKeyStatusDto> {
		...
		const recommendedUpload = isLow ? this.RECOMMENDED_PREKEYS - availableCount : 0;
	...
	async getBundlesForDevices(
		...
		this.logger.log(`Successfully built ${bundles.size}/${userDevices.length} key bundles`);
