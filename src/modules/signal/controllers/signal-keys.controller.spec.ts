...
describe('SignalKeysController', () => {
	...
	describe('getKeyBundleForDevice', () => {
		it('should return a key bundle for a specific device', async () => {
			...
			const result = await controller.getKeyBundleForDevice(mockUserId, mockDeviceId);

			expect(result).toEqual(mockBundle);
			expect(prKeyBundleService.getBundleForUser).toHaveBeenCalledWith(mockUserId, mockDeviceId);
		});
		...
		it('should throw NotFoundException if device has no keys', async () => {
			...
			await expect(controller.getKeyBundleForDevice(mockUserId, mockDeviceId)).rejects.toThrow(
				NotFoundException
			);
		});
	});
