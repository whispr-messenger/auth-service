...
export class NotificationService {
    ...
    async notifyNewDeviceLogin(data: NotificationData): Promise<void> {
        try {
            ...
            if (!user) {
                this.logger.warn(
                    `User not found for notification: ${data.userId}`
                )
                return
            }
    ...
    async notifyQRLogin(userId: string): Promise<void> {
        try {
            ...
            if (!user) {
                this.logger.warn(
                    `User not found for QR notification: ${userId}`
                )
                return
            }
    ...
    async notify2FAStatusChange(userId: string): Promise<void> {
        try {
            ...
            if (!user) {
                this.logger.warn(
                    `User not found for 2FA notification: ${userId}`
                )
                return
            }
    ...
    async notifyPasswordReset(phoneNumber: string): Promise<void> {
        try {
            ...
            this.logger.log(
                `Password reset notification sent to ${phoneNumber}`
            )
        } catch (error) {
            this.logger.error(
