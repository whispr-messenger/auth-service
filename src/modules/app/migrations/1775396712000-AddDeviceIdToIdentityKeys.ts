...
export class AddDeviceIdToIdentityKeys1775396712000 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		// Step 1: Delete all existing identity_keys since they don't have device_id
		// This is acceptable because identity keys are regenerated on next device registration
		await queryRunner.query(`
      DELETE FROM "auth"."identity_keys"
    `);

		// Step 2: Add device_id column to identity_keys table
		await queryRunner.query(`
      ALTER TABLE "auth"."identity_keys"
      ADD COLUMN "device_id" uuid NOT NULL
    `);

		// Step 3: Drop the old unique constraint on user_id only
		await queryRunner.query(`
      ALTER TABLE "auth"."identity_keys"
      DROP CONSTRAINT IF EXISTS "UQ_identity_keys_user_id"
    `);

		// Step 4: Create new unique constraint on user_id + device_id
		await queryRunner.query(`
      ALTER TABLE "auth"."identity_keys"
    ...
    `);
		...
		// Step 7: Remove the private_key_encrypted column as it's not used in the entity
		await queryRunner.query(`
      ALTER TABLE "auth"."identity_keys"
