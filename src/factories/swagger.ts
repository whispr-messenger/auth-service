...
function getSwaggerConfig(port: number, configService: ConfigService): SwaggerConfig {
	...
	return {
		...
		githubTokenUrl: configService.get<string>('GITHUB_OAUTH_TOKEN_URL', defaultGithubTokenUrl),
	};
}
...
function createSwaggerUiOptions(githubClientId: string, swaggerOAuthCallbackUrl: string): SwaggerUiOptions {
	return {
		initOAuth: {
