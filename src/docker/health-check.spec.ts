...
describe('health-check', () => {
    ...
    const mockHttpRequest = (
        response?: EventEmitter & http.IncomingMessage
    ) => {
        httpRequestSpy = jest
            .spyOn(http, 'request')
            .mockImplementation((options: any, callback?: any) => {
                if (callback && response) {
                    // Defer callback to next tick to simulate async behavior
    ...
    it('should exit with 0 when health check returns 200', async () => {
        ...
        expect(mockConsoleLog).toHaveBeenCalledWith(
            expect.stringContaining('Health check starting...')
        )
        expect(mockConsoleLog).toHaveBeenCalledWith(
    ...
    it('should exit with 1 when health check returns non-200 status', async () => {
        ...
        expect(mockConsoleError).toHaveBeenCalledWith(
            expect.stringContaining('Invalid status code 500')
        )
    })
    ...
    it('should exit with 1 when request times out', async () => {
        ...
        expect(mockConsoleError).toHaveBeenCalledWith(
            expect.stringContaining(
                '✗ Health check FAILED: Request timeout after 3000ms'
            )
        )
    ...
    it('should log correct request configuration', () => {
        ...
        expect(mockConsoleLog).toHaveBeenCalledWith(
            expect.stringContaining(
                'Target: GET http://localhost:3001/health/ready'
            )
        )
        expect(mockConsoleLog).toHaveBeenCalledWith(
            expect.stringContaining('Timeout: 3000ms')
        )
    })
