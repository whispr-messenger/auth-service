...
const req = http.request(options, (res: http.IncomingMessage) => {
  console.log(
    `[${timestamp()}] Response received with status code: ${res.statusCode}`
  );
  ...
  res.on('end', () => {
    ...
    if (res.statusCode === 200) {
      ...
      console.error(
        `[${timestamp()}] ✗ Health check FAILED: Invalid status code ${res.statusCode}`
      );
      process.exit(1);
    }
  });
...
req.on('timeout', () => {
  console.error(
    `[${timestamp()}] ✗ Health check FAILED: Request timeout after ${options.timeout}ms`
  );
  req.destroy();
  process.exit(1);
});
