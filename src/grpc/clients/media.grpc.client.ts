...
export class MediaGrpcClient implements OnModuleInit {
	...
	async getFileInfo(fileId: string, userId: string): Promise<any> {
		return new Promise((resolve, reject) => {
			...
			this.client.getFileInfo({ file_id: fileId, user_id: userId }, (error: any, response: any) => {
				if (error) {
	...
	async deleteFile(fileId: string, userId: string): Promise<any> {
		return new Promise((resolve, reject) => {
			...
			this.client.deleteFile({ file_id: fileId, user_id: userId }, (error: any, response: any) => {
				if (error) {
	...
	async generatePreview(
		...
		return new Promise((resolve, reject) => {
			...
			this.client.generatePreview(request, (error: any, response: any) => {
				if (error) {
	...
	async getPreview(fileId: string, userId: string, previewType: string = 'thumbnail'): Promise<any> {
		return new Promise((resolve, reject) => {
			if (!this.client) {
				return reject(new Error('gRPC client not initialized'));
			}
	...
	async updateFileMetadata(fileId: string, userId: string, metadata: any): Promise<any> {
		return new Promise((resolve, reject) => {
			if (!this.client) {
				return reject(new Error('gRPC client not initialized'));
			}
			...
			this.client.updateFileMetadata(request, (error: any, response: any) => {
				if (error) {
	...
	async getFileCategories(userId: string): Promise<any> {
		return new Promise((resolve, reject) => {
			...
			this.client.getFileCategories({ user_id: userId }, (error: any, response: any) => {
				if (error) {
					this.logger.error('Error getting file categories:', error);
					return reject(error);
				}
				resolve(response);
			});
	...
	async getStorageUsage(userId: string): Promise<any> {
		return new Promise((resolve, reject) => {
			...
			this.client.getStorageUsage({ user_id: userId }, (error: any, response: any) => {
				if (error) {
